...
export function myCommand() {
  ...
  vscode.window.showInputBox({ prompt: 'Enter the name of the file' }).then(async (fileName) => {
    ...
    if (fileName) {
      ...
      fs.writeFileSync(path.join(workspaceFolder.uri.fsPath, `${fileName}.jgen`), boilerplate);

      // Open the file in the editor
      vscode.workspace.openTextDocument(path.join(workspaceFolder.uri.fsPath, `${fileName}.jgen`)).then((doc) => {
